...
```
    ...
    section[data-testid="stSidebar"] {
        background: #FFFFFF;
        border-right: 1px solid rgba(255, 79, 123, 0.10);
    }
    ...
    .premium-card {
        ...
        border-radius: 28px;
        padding: 24px;
        box-shadow: 0 22px 50px rgba(255,79,123,0.10);
        height: 100%;
    }
...
current_date = date_key()
usage_id = f"{current_date}*{from_user_id}"
reaction_id = f"{from_user_id}*{to_user_id}"
...
final = (
    astro_component * 0.48
    + distance_component * 0.18
    + interest_component * 0.22
    ...
    st.markdown(
        ...
        <div class="auth-hero">
            ...
            <div class="hero-title">
                Aşkı biraz daha <span>uyumlu</span> hale getir.
            </div>
            <div class="hero-desc">
                LUNAMATCH; doğum haritanı, ilişki beklentilerini, yaşam tarzını ve
                ortak ilgi alanlarını bir araya getirerek sana daha anlamlı tanışmalar sunar.
                Fotoğrafa bak, uyumu gör, kalbinden geçen kişiye adım at.
            </div>
...
st.markdown(
    ...
    <div class="page-card">
        ...
        <div class="section-subtitle">
            Daha iyi eşleşmeler için kendini doğal, sıcak ve samimi bir şekilde anlat.
        </div>
    </div>
    """,
    unsafe_allow_html=True,
...
with st.form("profile_form"):
    ...
    with col1:
        display_name = st.text_input(
            "Profilde görünecek adın",
            value=existing.get("display_name", ""),
        ...
        gender = st.selectbox(
            ...
            GENDER_OPTIONS,
            index=GENDER_OPTIONS.index(existing.get("gender", NO_ANSWER))
            if existing.get("gender", NO_ANSWER) in GENDER_OPTIONS else 0,
        )

    with col2:
        city = st.text_input(
            "Hangi şehirde yaşıyorsun?",
            value=existing.get("city", "İstanbul"),
        )
        looking_for = st.selectbox(
            "Kimlerle tanışmak istersin?",
            LOOKING_FOR_OPTIONS,
            index=LOOKING_FOR_OPTIONS.index(existing.get("looking_for", "Herkes"))
            if existing.get("looking_for", "Herkes") in LOOKING_FOR_OPTIONS else 0,
        )
        relationship_intent = st.selectbox(
            "Şu an ne arıyorsun?",
            RELATIONSHIP_INTENT_OPTIONS,
            index=RELATIONSHIP_INTENT_OPTIONS.index(existing.get("relationship_intent", "Ciddi ilişki"))
            if existing.get("relationship_intent", "Ciddi ilişki") in RELATIONSHIP_INTENT_OPTIONS else 0,
        )
    ...
    with col3:
        birth_date = st.date_input(
            "Doğum tarihin",
            value=parse_date(existing.get("birth_date"), date(1998, 1, 1)),
        )

    with col4:
        moon_sign = st.selectbox(
            "Ay burcun",
            ZODIAC_SIGNS,
            index=sign_index(existing.get("moon_sign")),
            format_func=sign_label,
        )

    with col5:
        rising_sign = st.selectbox(
            "Yükselen burcun",
            ZODIAC_SIGNS,
            index=sign_index(existing.get("rising_sign")),
            format_func=sign_label,
        )
    ...
    three_words = st.text_input(
        ...
        value=existing.get("three_words", ""),
        placeholder="Örn. Samimi, meraklı, romantik",
    )

    first_date = st.text_input(
        "İlk buluşmada nasıl bir ortam seni mutlu eder?",
        value=existing.get("first_date", ""),
        placeholder="Örn. Sakin bir kahveci, güzel bir yürüyüş...",
    )

    relationship_value = st.text_input(
        "Bir ilişkide kalbini en çok ne etkiler?",
        value=existing.get("relationship_value", ""),
        placeholder="Örn. Güven, mizah, sadakat, birlikte gelişmek...",
    )

    partner_expectations = st.text_input(
        "Yanındaki kişide en çok ne ararsın?",
        value=existing.get("partner_expectations", ""),
        placeholder="Örn. İçtenlik, anlayış, neşe...",
    )

    love_language = st.selectbox(
        "Aşk dilin hangisine daha yakın?",
        LOVE_LANGUAGE_OPTIONS,
        index=LOVE_LANGUAGE_OPTIONS.index(existing.get("love_language", NO_ANSWER))
        if existing.get("love_language", NO_ANSWER) in LOVE_LANGUAGE_OPTIONS else 0,
    )

    energy_style = st.selectbox(
        "Seni en iyi anlatan enerji hangisi?",
        ENERGY_OPTIONS,
        index=ENERGY_OPTIONS.index(existing.get("energy_style", NO_ANSWER))
        if existing.get("energy_style", NO_ANSWER) in ENERGY_OPTIONS else 0,
    )

    weekend_style = st.text_input(
        "Boş bir pazar gününü nasıl geçirmek istersin?",
        value=existing.get("weekend_style", ""),
        placeholder="Örn. Geç kahvaltı, sahil yürüyüşü ve film...",
    )
    ...
    with col6:
        location_lat = st.number_input(
            ...
            help="İstanbul için varsayılan değer kullanıldı.",
        )
    ...
    with col9:
        openness = st.slider(
            ...
            1,
            10,
            int(existing.get("openness", 7) or 7),
        )

    with col10:
        emotionality = st.slider(
            "Duygusal yakınlık ihtiyacı",
            1,
            10,
            int(existing.get("emotionality", 6) or 6),
        )

    with col11:
        extroversion = st.slider(
            "Sosyallik enerjin",
            1,
            10,
            int(existing.get("extroversion", 5) or 5),
        )
...
if not user.get("profile_completed"):
    st.markdown(
        ...
        <div class="page-card">
            ...
            <div class="section-subtitle">
                Sana uygun profilleri gösterebilmemiz için önce profilini tamamlamalısın.
            </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    if st.button("👤 Profilimi Oluştur", use_container_width=True):
        st.session_state["main_nav"] = "👤 Profilim"
        st.rerun()
...
st.markdown(
    ...
    <div class="page-card">
        ...
        <div class="section-subtitle">
            {escape(weather.focus_text)} Bugünün şanslı burçları: <b>{escape(lucky)}</b>
        </div>
    </div>
    """,
    unsafe_allow_html=True,
...
if not candidates:
    st.markdown(
        ...
        <div class="page-card">
            ...
            <div class="section-subtitle">
                Şu an kriterlerine uygun yeni profil görünmüyor. Yeni üyeler geldikçe burada belirecek.
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
if not user.get("is_premium") and remaining == 0:
    st.markdown(
        ...
        <div class="page-card">
            ...
            <div class="section-subtitle">
                Premium ile sınırsız kalp gönderebilir, süper uyumları kullanabilir ve daha fazla profili keşfedebilirsin.
            </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    if st.button("💎 Premium’u İncele", use_container_width=True):
        st.session_state["main_nav"] = "💎 Premium"
        st.rerun()
...
interests_html = "".join(
    f'<span class="interest-badge">{escape(interest)}</span>'
    for interest in candidate.get("interests", [])[:8]
)
...
reasons_html = "<br>".join(
    f"• {escape(reason)}"
    for reason in score.get("daily_reasons", [])
)
    ...
    st.markdown(
        ...
        <div class="profile-info">
            ...
            <div class="profile-meta">
                {escape(candidate.get("city"))} · {escape(candidate.get("relationship_intent"))}
            </div>
            <div class="compat-score">💘 %{score["adjusted_astro_score"]} LUNAMATCH uyumu</div>
            <div>{top_pick}</div>
            <div style="margin-top:10px;">{signs_html}</div>
            <p style="font-size:16px; color:#5E4A58; line-height:1.65; margin-top:18px;">
                “{escape(candidate.get("about_text", candidate.get("three_words", "")))}”
            </p>
...
st.markdown(
    ...
    <div class="page-card">
        ...
        <div class="section-subtitle">
            Karşılıklı kalp gönderdiğin kişiler burada görünür.
        </div>
    </div>
    """,
    unsafe_allow_html=True,
    ...
    with col2:
        st.markdown(
            ...
            <div class="match-card">
                <div style="font-size:24px; font-weight:900; color:#2B1B2E;">
                    {escape(match.get("display_name"))}, {escape(match.get("age"))}
                </div>
                <div style="color:#7A6B76; margin:6px 0 12px 0;">
                    {escape(match.get("city"))} · ☀️ {escape(sign_label(match.get("sun_sign")))}
                </div>
                <div style="color:#5E4A58; line-height:1.6;">
                    {escape(match.get("about_text", ""))}
                </div>
            </div>
            """,
            unsafe_allow_html=True,
...
st.markdown(
    ...
    <div class="page-card">
        ...
        <div class="section-subtitle">
            Daha fazla görünürlük, sınırsız kalp ve daha özel astro uyum deneyimi.
        </div>
    </div>
    """,
    unsafe_allow_html=True,
...
st.markdown(
    ...
    <div class="page-card">
        ...
        <div class="section-subtitle">
            Kullanıcıları, rollerini ve premium durumlarını buradan yönetebilirsiniz.
        </div>
    </div>
    """,
    unsafe_allow_html=True,
...
with col1:
    new_role = st.selectbox(
        "Rol",
        ["user", "admin"],
        index=0 if selected.get("role") != "admin" else 1,
    )

with col2:
    premium_status = st.checkbox(
        "Premium aktif",
        value=bool(selected.get("is_premium")),
    )
